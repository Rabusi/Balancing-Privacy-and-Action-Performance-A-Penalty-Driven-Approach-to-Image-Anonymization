...
class reconstruction_dataset(Dataset):
    def __init__(self, data_split, shuffle=True, ucf101_percentage=1.0, data_percentage=1.0):
        ...
        ucf101_path = os.path.join(cfg.ucf101_path, f'{self.data_split}')
        ucf101_data_list = glob.glob(os.path.join(ucf101_path, '*', '*', '*.png'))
    ...
    def build_image(self, img_path):
        try:
            ...
            if not img.shape[0] == 3:
                logging.info(f'{img_path} has {img.shape[0]} channels')
                return None
            ...
            try:
                ...
                logging.info(f'Image {img_path} Failed')
                return None   
...
if __name__ == '__main__':
    ...
    logging.info(f'Step involved: {len(train_dataset)/params.batch_size}')
    t = time.time()

    for i, (img, img_path) in enumerate(train_dataloader):
        if i%10 == 0:
            logging.info()
            logging.info(f'Image shape is {img.shape}')

    logging.info(f'Time taken to load data is {time.time()-t}')
