...
from aux_code.vispr_dl_edit import vispr_dataset
# from aux_code.pahmdb_private_dl import pahmdb_private_dataset
...
def val_epoch(epoch, validation_dataloader, fa_model, fb_model, anon, criterion, use_cuda, writer):
    ...
    for i, (inputs, label, vid_path) in enumerate(validation_dataloader):
        if len(inputs.shape) != 1:
            ...
            # Set the last element of each prediction to zero
            # output[:, -1] = 0             # for pahmdb
    ...
    # return pred_dict, label_dict, np.mean(ap[:6])   # for pahmdb
    

# Main code loop.
def train_classifier(params, arch):
    ...
    fa_model = load_fa_model(arch='unet++', saved_model_file=params.saved_model) #UNet(n_channels = 3, n_classes=3)
    # Freeze model weights.
    for param in fa_model.parameters():
        param.requires_grad = False
    
    # Initialize privacy prediction model.
    fb_model = load_fb_model(arch=params.arch, saved_model_file=None, ssl=False, pretrained=False, num_pa=params.num_pa)
    epoch1 = 1
    ...
    # validation_dataset = pahmdb_private_dataset(shuffle=False, data_percentage=1.0)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--run_id", dest='run_id', type=str, required=False, default='params_privacy.py', help='params')
    parser.add_argument("--arch", dest='arch', type=str, required=False, default='r50', help='model architecture')

    args = parser.parse_args()

    train_classifier(params, args.arch)
