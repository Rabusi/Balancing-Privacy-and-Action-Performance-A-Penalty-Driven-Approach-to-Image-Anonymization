...
class vispr_dataset(Dataset):
    def __init__(self, data_split, shuffle=True, data_percentage=1.0):
        ...
        self.labels = pickle.load(open(os.path.join(cfg.vispr_path , f'{self.data_split}_VISPR1_labels.pkl'), 'rb'))
        # self.labels = pickle.load(open(os.path.join(cfg.vispr_path , f'{self.data_split}_VISPR2_labels.pkl'), 'rb'))
        data_path = os.path.join(cfg.vispr_path, f'{self.data_split}')  
    ...
    def build_image(self, img_path):
        try:
            img = torchvision.io.read_image(img_path, mode=torchvision.io.image.ImageReadMode.RGB)
