        ...
        for action_name, class_id in class_labels.items():
            ...
            if os.path.exists(train_list_path):
                with open(train_list_path, 'r') as action_file:
                    ...
                    self.all_paths.extend(split_paths)
            else:
...
# Validation dataset.
class single_val_dataloader_pahmdb(Dataset):
    ...
    def __init__(self, params, action_name, shuffle=True, data_percentage=1.0, mode=0, \
        ...
        self.all_paths = []  # all the test path from PA-HMDB
        
        with open(action_name, "r") as f:
            self.actions = json.load(f)
            
        PAHMDB_test_path = os.path.join(cfg.pahmdb_path)

        for action in os.listdir(PAHMDB_test_path):
            action_path = os.path.join(PAHMDB_test_path, action)
            if os.path.isfile(action_path) and action.endswith('.json'):
                with open(action_path, 'r') as f:
                    json_data = json.load(f)
                    
                    action_name = action.replace('.json', '')
                    
                    for avi_file, attributes in json_data.items():
                        if avi_file.endswith('.avi'):
                            avi_file_path = os.path.join(cfg.hmdb51_path, action_name, avi_file)
                            self.all_paths.append(avi_file_path)
...
# Training dataloader.
class contrastive_train_dataloader_hmdb(Dataset):
    def __init__(self, params, action_name, shuffle=True, data_percentage=1.0, frame_wise_aug=False, split = 1):
        self.params = params
        self.all_paths = []
        ...
        self.data_percentage = data_percentage
        self.data_limit = int(len(self.all_paths) * self.data_percentage)
        self.data = self.all_paths[:self.data_limit]
        self.PIL = trans.ToPILImage()
        self.TENSOR = trans.ToTensor()
        self.erase_size = 19
        self.framewise_aug = frame_wise_aug
...
# Validation dataset.
class contrastive_val_dataloader_hmdb(Dataset):

    def __init__(self, params, action_name, shuffle=True, data_percentage=1.0, mode=0, hflip=0, cropping_factor=0.8, threeCrop=False, split = 1):
    ...
    def process_data(self, idx):
        vid_path = os.path.join(cfg.hmdb51_path, self.data[idx].split(' ')[0])
        label = self.classes[vid_path.split(os.sep)[-2]]
...
if __name__ == '__main__':
    ...
    train_dataset = single_train_dataloader_hmdb(params=params, shuffle=False, data_percentage=0.1)
